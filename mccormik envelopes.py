...
# Define Variables
model.teste = Var(within = Reals)
model.x = Var(within = Reals)
# model.x = Param(initialize = 10)
...
def constraint_test(model):
    return model.teste == 1 / model.x
model.constraint_test = Constraint(rule = constraint_test)
...
print(f"type x = {type(model.x).__name__}")

if type(model.x).__name__ == 'ScalarParam':
    print('yes')