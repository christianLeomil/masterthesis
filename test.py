...
class charging_station:
    ...
    def __init__(self,name_of_instance):
        ...
        self.dict_parameters  = {'list_sheets':['other','other','other','other','other','other','other'],
                                 ...
                                 'internal_time_step': 0.5, #time step in hours
                                 'time_step': 1, #time step in hours
                                 f"{self.name_of_instance}_charging_points" : 2}
    ...
    def charging_demand_calculation(self):
        ...
        for i in range(0 ,int(self.dict_parameters['number_hours'] * self.dict_parameters['number_hours'] / self.dict_parameters['internal_time_step'])):
            delta = timedelta(hours = i * self.dict_parameters['internal_time_step'])
            converted_time_stamp = self.dict_parameters['reference_date'] + delta
            converted_time_stamp = converted_time_stamp.strftime('%Y-%m-%d %H:%M')
        ...
        for i in range(1,len(list_time)):
            df_temp = df_schedule[(df_schedule['time stamp'] >= list_time[i-1]) & (df_schedule['time stamp'] < list_time[i])].copy()
            # condition to limit number of cars charging at the same time in charging station
            if len(df_temp) > self.dict_parameters[f"{self.name_of_instance}_charging_points"]: 
                df_temp.sort_values(by = 'time stamp', ascending = True, inplace = True)
                df_temp = df_temp.head(self.dict_parameters[f"{self.name_of_instance}_charging_points"])
        ...
        df_structured = pd.DataFrame({'time stamp':list_time,
                                      'models':list_models,
                                      'start SoC':list_start_SoC,
                                      'end SoC':list_end_SoC})
        
        # df_structured.to_excel(self.path_output + 'teste.xlsx')
        ...
        for i in df_structured.index:
            ...
            for j in range(0,len(models)):
                ...
                power = (end_SoC - start_SoC) * capacity / self.dict_parameters['internal_time_step']
        ...
        list_power = []
        for i in range(1,len(list_time)):
            df_temp = df_structured[(df_structured['time stamp'] >= list_time[i-1]) & (df_structured['time stamp'] < list_time[i])].copy()
            list_power.append(df_temp['total power'].sum())

        list_time = list_time[:-1]
        df_structured_time_step = pd.DataFrame({'hours':list_time,
                                                'total power':list_power})
        
        df_structured_time_step.to_excel(self.path_output + 'df_structured_time_step.xlsx', index = False)

        return df_structured_time_step['total power'].tolist()
...
charging_station1 = charging_station('charging_station1')