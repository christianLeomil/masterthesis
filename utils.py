...
def create_connection_equations(df_domains,control):
    ...
    for i in df_domains.index:
        ...
        list_sub = [s.replace(domain_name + '_to_charging_station','param_demand_charging_station') for s in list_sub]
            ...
            for k in df_connect_stock.index:
                if df_connect_stock.loc[k,j] != 0:
                    if df_stock.loc[k,j] != 0:
                        ...
                        list_variables_rev.append(name_variable)
                        list_expressions_rev.append(F"model.{name_variable}[t] == model.param_{k}_stock_price[t] * model.{name_energy_flow}[t] * model.time_step")
